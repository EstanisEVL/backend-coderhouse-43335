      ...
      if (!checkUser) {
        ...
        if (checkUser.carts.length === 0) {
          try {
            ...
            checkUser.last_connection = `Última conexión: ${new Date().toLocaleString(
              "es-ES",
              {
...
export const userLogout = async (req, res) => {
  try {
    if (!req.user) {
      ...
      const user = req.user;
      user.user.last_connection = `Última desconexión: ${new Date().toLocaleString(
        "es-ES",
        {
          timeZone: "America/Argentina/Buenos_Aires",
        }
      ...
      const checkUser = await UserService.findUser(null, user.user.id);
      checkUser.last_connection = user.user.last_connection;
      await checkUser.save();

      res.clearCookie("Cookie");
      return res.status(200).json({ message: "Usuario desconectado, ", user });
      // redirect("/login");
    }
  } catch (err) {
...
export const getGithubUser = async (req, res) => {
  try {
    ...
    if (githubUser.carts.length === 0) {
      ...
      githubUser.last_connection = `Última conexión: ${new Date().toLocaleString(
        "es-ES",
        {
          timeZone: "America/Argentina/Buenos_Aires",
        }
